r"""
Job status worker launcher.

Starts a ``JobStatusWorkerRunner`` that observes all messages on the
fanout exchange and records per-document processing results.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start a job status worker",
    )
    parser.add_argument(
        "--pipeline-spec",
        type=str,
        ...
        help=(
            ...
            "resolve the exchange to observe (default_exchange). "
...
async def start_job_status_worker(
    *,
    ...
    await run(
        runner_factory_func=partial(JobStatusWorkerRunner, exchange=exchange),
        config_file=resolved_config,
        worker_name=worker_name,
        worker_name_prefix="job-status-worker",
        debug=debug,
        logger=log,
    )
...
def main() -> None:
    ...
    try:
        ...
        module_logger.error(
            f"Start job status worker failed: {summarize_exception_chain(e)}"
        )
        sys.exit(1)
