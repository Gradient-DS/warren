...
**Fanout is the default, and the simplest — for many pipelines it's all you need.** Every worker sees every message and self-selects, so building a pipeline is just adding workers; there's no routing to configure. Start here. When a concrete need appears — routing heterogeneous inputs by kind, or letting each job pick its own path through the workers — the same runtime and launcher scripts also support **topic** and **direct** exchanges. See [`warren/docs/routing.md`](../docs/routing.md) for *Choosing an exchange* and the full routing model.
